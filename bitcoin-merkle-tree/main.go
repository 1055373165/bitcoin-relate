...
func main() {
	...
	//递归层次遍历默克尔树,与此同时,可以返回各层的内部hash值,由此可以构造merkle branch
	fmt.Println("层序遍历结果(递归):")
	fmt.Println()
	fmt.Println("_________________________________________")
	fmt.Println()
	travel_layer_recur(root.RootNode, 0)
	//遍历内部hash结果切片,输出每层的hash值
...
// 从full node那里获取getMerkleBranch
func getMerkleBranch(hash []byte) [][]byte {
	ans := [][]byte{} //存储各层的内部hash组成Merkle Branch
	index := -1       // 用来记录匹配给定hash的位置，-1表示没找到
	/* 首先处理最后一层的8个值,存在结果切片的末尾 */
	layer := res[len(res)-1] //[1 2 3 4 5 6 7 8]
	//找给定hash的位置
